...
(function ($) {
    ...
    VectorCanvas.prototype = {
        ...
        createPath: function createPath(config) {
            var node;
            if (this.mode == 'svg') {
        ...
        createGroup: function createGroup(isRoot) {
            var node;
            if (this.mode == 'svg') {
        ...
        applyTransformParams: function applyTransformParams(scale, transX, transY) {
            if (this.mode == 'svg') {
    ...
    var WorldMap = function WorldMap(params) {
        params = params || {};
        var map = this;
    ...
    WorldMap.prototype = {
        ...
        setColors: function setColors(key, color) {
        ...
        setScaleColors: function setScaleColors(colors) {
        ...
        highlight: function highlight(cc, path) {
        ...
        unhighlight: function unhighlight(cc, path) {
            path = path || $('#' + this.getCountryId(cc))[0];
            path.setOpacity(1);
        ...
        select: function select(cc, path) {
            path = path || $('#' + this.getCountryId(cc))[0];
        ...
        deselect: function deselect(cc, path) {
            path = path || $('#' + this.getCountryId(cc))[0];
            if (this.selectedRegions.indexOf(cc) >= 0) {
        ...
        isSelected: function isSelected(cc) {
        ...
        getCountryId: function getCountryId(cc) {
            return 'jqvmap' + this.index + '_' + cc;
        },

        getPinId: function getPinId(cc) {
            return this.getCountryId(cc) + '_pin';
        },

        placePins: function placePins(pins, pinMode) {
            var map = this;
        ...
        positionPins: function positionPins() {
            var map = this;
        ...
        removePin: function removePin(cc) {
...
(function ($) {
    ...
    var defaults = {
        ...
        subCenterClass: 'subcenter-slide',
        currIndex: 3
    };
    ...
    $.fn.nvslider = function (options) {
        ...
        var methods = {
            init: function init() {
                node.wrap = el;
                node.slides = node.wrap.children();

                //
                node.wrap.append('<div class="nv-slider__rails"></div>');
                node.rails = node.wrap.children('.nv-slider__rails');
                node.slides.detach().appendTo(node.rails);

                node.wrap.prepend('<div class="nv-slider__arrow nv-slider__prev"></div>');
                node.wrap.append('<div class="nv-slider__arrow nv-slider__next"></div>');

                node.prev = node.wrap.children('.nv-slider__prev');
                node.next = node.wrap.children('.nv-slider__next');

                //
                node.wrap.addClass('nv-slider');
                node.slides.addClass('nv-slider__item');

                //data
                methods.checkPosition(settings.currIndex - 1);

                node.slides.eq(data.currIndex).addClass(settings.centerClass);
                node.slides.eq(data.prevIndex).addClass(settings.subCenterClass);
                node.slides.eq(data.nextIndex).addClass(settings.subCenterClass);
                ...
                data.currPosition = Number(node.rails.css('transform').split(',')[4]);
            ...
            bindControls: function bindControls() {
                node.prev.bind('click', methods.prevSlide);
                node.next.bind('click', methods.nextSlide);

                node.slides.eq(data.prevIndex).bind('click', methods.prevSlide);
                node.slides.eq(data.nextIndex).bind('click', methods.nextSlide);
            },

            unBindControls: function unBindControls() {
                node.prev.unbind('click', methods.prevSlide);
                node.next.unbind('click', methods.nextSlide);

                node.slides.eq(data.prevIndex).unbind('click', methods.prevSlide);
                node.slides.eq(data.nextIndex).unbind('click', methods.nextSlide);
            },

            infiniteInit: function infiniteInit() {
                node.firstSlide = node.slides.first().clone(true, true);
                node.lastSlide = node.slides.last().clone(true, true);

                node.rails.append(node.firstSlide);
                node.rails.prepend(node.lastSlide);

                node.firstSlide = node.rails.children().first();
                node.lastSlide = node.rails.children().last();

                data.difference = data.currPosition - node.firstSlide.outerWidth();

                //
                node.rails.css({
                    'transform': 'translate(' + data.difference + 'px)'
                });

                node.slides = node.rails.children();
            },

            removeClones: function removeClones() {
                node.firstSlide.remove();
                node.lastSlide.remove();
            },

            checkPosition: function checkPosition(index) {

                data.slideWidth = node.slides.eq(data.nextIndex + 1).outerWidth();

                if (index >= 0 && index < node.slides.length) {
                    data.currIndex = index;
                    data.prevIndex = data.currIndex - 1;
                    data.nextIndex = data.currIndex + 1;

                    data.prevIndex < 0 ? data.prevIndex = node.slides.length - 1 : '';
                    data.nextIndex >= node.slides.length ? data.nextIndex = 0 : '';
                } else if (index < 0) {
                    data.currIndex = node.slides.length - 1;
                    data.prevIndex = 0;
                    data.nextIndex = node.slides.length - 2;
                } else if (index >= node.slides.length) {
                    data.currIndex = 0;
                    data.prevIndex = node.slides.length - 1;
                    data.nextIndex = 1;
                }
            },
            ...
            changeSlide: function changeSlide() {
                ...
                node.slides.eq(data.currIndex).addClass(settings.centerClass).siblings().removeClass(settings.centerClass);
                node.slides.removeClass(settings.subCenterClass);
                node.slides.eq(data.prevIndex).addClass(settings.subCenterClass);
                node.slides.eq(data.nextIndex).addClass(settings.subCenterClass);
            ...
            prevSlide: function prevSlide(e) {
                ...
                methods.unBindControls();
                methods.checkPosition(index);
                data.difference = data.currPosition + data.slideWidth;
                methods.changeSlide();
                e.preventDefault();
            },
